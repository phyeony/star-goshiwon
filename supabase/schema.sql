...
CREATE INDEX idx_booking_requests_assigned_room_unit_id ON booking_requests(assigned_room_unit_id);
...
CREATE INDEX idx_room_units_room_id ON room_units(room_id, sort_order);
CREATE INDEX idx_room_units_status ON room_units(status);

CREATE TABLE room_unit_blocks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_room_unit_blocks_room_unit_dates
  ON room_unit_blocks(room_unit_id, check_in_date, check_out_date);
CREATE INDEX idx_room_unit_blocks_booking_request_id
  ON room_unit_blocks(booking_request_id);
CREATE INDEX idx_room_unit_blocks_status ON room_unit_blocks(status);
...
-- Guest reviews (2026-07-06): admin-minted single-use invite links + moderated reviews.
